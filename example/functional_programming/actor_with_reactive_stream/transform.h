...
auto transform(Sender &&sender, Transformation &&transformation) {
    return detail::transform_impl<Sender, Transformation>(
        std::forward<Sender>(sender), std::forward<Transformation>(transformation));
}
...
auto transform(Transformation &&transformation) {
    return detail::transform_helper<Transformation>{std::forward<Transformation>(transformation)};
}
